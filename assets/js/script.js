...
// check output capitalize (repeat checkEmptiness function counter)
function checkCapital(out) {
    Cap = true;
    if (out !== 'λ') {
        for (let i = 0; i < out.length; i++) {
            if (lowerCase.test(out[i])) {
                Cap = false;
                return Cap
            }
        }
...
function getFirst(grammar) {
    ...
    for (let x = 0; x < firstFollowLoop; x++) { // change this while condition Todo
        for (let i = 0; i < starters.length; i++) {
            Capital = false;
...
function getFollow(grammar) {
    ...
    for (let x = 0; x < firstFollowLoop; x++) {
        followList = [];
        operation = [];
        follow = "";
        tmp = "";
...
// check if any var available in first|follow dictionary
function check_var_exist(first) {
    let tmp = {};
    if (first) {
        tmp = firsts;
    } else {
    ...
    for (let i = 0; i < starters.length; i++) {
        start = starters[i];
        for (let j = 0; j < tmp[start].length; j++) {
            if (UpperCase.test(tmp[start][j])) {
                firstFollowLoop++;
            }
        }
    }
...
function showResult() {
    ...
    textFollow = "";
    for (let i = 0; i < starters.length; i++) {
        text += "first(" + starters[i] + ") = {" + firsts[starters[i]].filter((a) => a) + "} <br>";
        textFollow += "follow(" + starters[i] + ") = {" + follows[starters[i]].filter((a) => a) + "} <br>";
    }
    characterFirst = 0;
    characterFollow = 0;
